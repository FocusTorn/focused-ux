...
//= IMPLEMENTATIONS ===========================================================================================
import type { OptimizeIconsResult } from './generate_optimized_icons.js'
...
function formatOptimizationResults(result: OptimizeIconsResult): void { //>
	console.log(`\n${ansii.blueLight}Optimization Results:${ansii.none}`)
	
	if (result.filesAttempted && result.fileOptimizationDetails.length > 0) {
		console.log(`  ${ansii.gold}Files:${ansii.none}`)
		result.fileOptimizationDetails.forEach((detail) => {
			console.log(`    ${detail.fileName}: ${detail.originalSize} → ${detail.optimizedSize} (${detail.reduction} bytes, ${detail.percentage}%)`)
		})
	}
	
	if (result.foldersAttempted && result.folderOptimizationDetails.length > 0) {
		console.log(`  ${ansii.gold}Folders:${ansii.none}`)
		result.folderOptimizationDetails.forEach((detail) => {
			console.log(`    ${detail.fileName}: ${detail.originalSize} → ${detail.optimizedSize} (${detail.reduction} bytes, ${detail.percentage}%)`)
		})
	}
} //<
...
async function run(): Promise<void> { //>
	...
	if (hasIconsToProcess) {
		...
		formatOptimizationResults(optResult)
