...
class AssetProcessingOrchestrator {
	...
	async processAssets(): Promise<void> {
		...
		// Change to scripts directory for proper import resolution
		...
		try {
			...
			console.log('🔄 [Step 1: Importing from external source...]')

			const importResult = await this.importFromExternalSource()
			
			if (!importResult.success) {
				console.log('⚠️  External source not available, continuing with existing assets')
			} else {
				console.log(`✅ Imported ${importResult.importedCount} new SVG icons`)
			}

			// Step 2: Optimize icons (if any exist)
			console.log('🔄 [Step 2: Optimizing icons...]')

			const optimizationResult = await this.optimizeIcons()
			
			if (optimizationResult.success) {
				console.log(`✅ Icons optimized: ${optimizationResult.optimizedCount} processed`)
			}

			// Step 3: Generate theme files
			console.log('🔄 [Step 3: Generating theme files...]')

			const themeResult = await this.generateThemeFiles()
			...
			console.log('🔄 [Step 4: Creating preview images...]')

			const previewResult = await this.generatePreviews()
			...
			console.log('\n🔄 [Step 5: Running asset validation...]')

			const validationResult = await this.validateExistingAssets()
			...
			if (validationResult) {
				...
				if (validationResult.warnings && validationResult.warnings.length > 0) {
					...
					const orphanedFileIcons = validationResult.warnings.filter(w =>
						w.code === 'ORPHANED_FILE_ICON',
					)
					const orphanedFolderIcons = validationResult.warnings.filter(w =>
						w.code === 'ORPHANED_FOLDER_ICON',
					)
					const duplicateNames = validationResult.warnings.filter(w =>
	...
	/**
	 * Import SVG icons from external source
	 */
	private async importFromExternalSource(): Promise<{ success: boolean, importedCount: number }> {
		try {
			const { main: importMain } = await import('./build_dynamicon_assets.js')

			// The import function will handle copying from external to internal
			await importMain('all')
			
			return { success: true, importedCount: 1 } // TODO: Get actual count
		} catch (error) {
			console.log(`⚠️  Import failed: ${error}`)
	 ...
	 * Optimize SVG icons
	 */
	private async optimizeIcons(): Promise<{ success: boolean, optimizedCount: number }> {
		try {
			const { main: optimizeMain } = await import('./generate_optimized_icons.js')
			const result = await optimizeMain('all', true)
			
			return { success: true, optimizedCount: result.filesAttempted + result.foldersAttempted }
		} catch (error) {
			console.log(`⚠️  Icon optimization failed: ${error}`)
			return { success: false, optimizedCount: 0 }
		}
	}

	/**
	 * Generate theme files from models
	 */
	private async generateThemeFiles(): Promise<{ success: boolean }> {
		try {
			const { main: generateMain } = await import('./generate_icon_manifests.js')
			const success = await generateMain(false)
			
			return { success }
		} catch (error) {
			console.log(`⚠️  Theme generation failed: ${error}`)
	 ...
	 */
	private async generatePreviews(): Promise<{ success: boolean }> {
		try {
			const { main: previewMain } = await import('./generate_icon_previews.js')
			const success = await previewMain('all', true)
	...
	/**
	 * Validate existing assets even when no changes are detected
	 */
	private async validateExistingAssets(): Promise<any> {
		try {
			// Create a minimal manifest for validation
			const minimalManifest = { assets: [], categories: {}, dependencies: {}, version: '', generatedAt: 0 }
			
			// Create a validator instance with minimal manifest
			const validator = new AssetValidator('assets', 'src/models', 'assets/themes', minimalManifest)
			
			// Validate existing assets in the output directory
			const validationResult = await validator.validateAssets(true) // true for verbose
