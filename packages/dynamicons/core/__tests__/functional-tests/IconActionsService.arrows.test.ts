...
describe('IconActionsService - Arrows', () => {
	...
	describe('toggleExplorerArrows', () => {
		it('should toggle arrows from false to true', async () => {
		...
		it('should toggle arrows from true to false', async () => {
			// Mock the regenerateAndApplyTheme method to avoid complex implementation
			const originalRegenerateAndApplyTheme = (service as any).regenerateAndApplyTheme
		...
		it('should handle configuration update error', async () => {
			(mockConfigService.getHideArrowsSetting as any).mockResolvedValue(false)
			;(mockConfigService.updateHideArrowsSetting as any).mockRejectedValue(new Error('Update failed'))

			await expect(service.toggleExplorerArrows()).rejects.toThrow('Update failed')
		})
	})
})
