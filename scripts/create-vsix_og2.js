...
try {
	...
	// Create a temporary .vscodeignore that ensures dist/ and node_modules/ are INCLUDED
	// We filter out any lines that exclude these paths in the staged copy
	const vscodeignorePath = join(deployDir, '.vscodeignore')
	const originalVscodeignore = existsSync(vscodeignorePath)
		...
		const filtered = originalVscodeignore
			...
			.filter((line) => {
				...
				if (trimmed === '' || trimmed.startsWith('#'))
					return true
				// Drop rules that exclude dist or node_modules
				if (/^dist\/?(\*\*)?$/.test(trimmed))
					return false
