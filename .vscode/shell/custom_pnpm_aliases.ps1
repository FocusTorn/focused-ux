...
# │        dc a                                                                                                        │
# │        -> tsx scripts/audit-feature-structure.ts dynamicons                                                        │
# │                                                                                                                    │
# │ CONFIGURATION                                                                                                      │
...
function _Invoke-PnpmPackageManagement { #>
    ...
    if ($CommandArgs.Count -lt 2) { Write-Host "Please provide a package to $($CommandArgs)."; return }
...
function Expand-NxTargetAlias { #>
    ...
    if ($script:NxTargetShortcuts.ContainsKey($first)) {
        ...
        $InputArgs = $script:NxTargetShortcuts[$first]
    } else {
...
function Invoke-NxCommand { #>
    ...
    if ($argsWithoutDebug.Count -eq 1 -and $argsWithoutDebug -eq 'ev') {
        $packageAliases = Get-PnpmAliasCache
    ...
    if ($Alias -in 'ext', 'core', 'all') {
        if ($finalArgs.Count -gt 0 -and $finalArgs -eq 'audit') {
            if ($IsDebug) { Write-Host "$($PSStyle.Dim)  - Routing to: Full Audit Script$($PSStyle.Reset)" }
            $auditArgs = @('libs/tools/structure-auditor/src/main.ts')
            _Invoke-Process -Executable 'tsx' -PreArgs $auditArgs -IsDebug:$isDebug
        } else {
    ...
    if ($firstArg -eq 'audit') {
        if ($IsDebug) { Write-Host "$($PSStyle.Dim)  - Routing to: Single Package Audit Script$($PSStyle.Reset)" }
        $packageDirName = $null
        if ($nxProjectName -match "@fux/(.+?)(?:-core|-ext)?$") {
            $packageDirName = $Matches[1]
        }
        if (-not $packageDirName) {
            Write-Error "Could not determine package directory name for alias '$Alias' ($nxProjectName)."; return
        }
        $auditArgs = @('libs/tools/structure-auditor/src/main.ts', $packageDirName)
        _Invoke-Process -Executable 'tsx' -PreArgs $auditArgs -IsDebug:$isDebug
    } elseif ($firstArg -in 'add', 'remove') {
...
#         if ($finalArgs.Count -gt 0 -and $finalArgs -eq 'audit') {
#             if ($IsDebug) { Write-Host "$($PSStyle.Dim)  - Routing to: Full Audit Script$($PSStyle.Reset)" }
#             $auditArgs = @('audit', 'structure-auditor')
#             _Invoke-Process -Executable 'nx' -PreArgs $auditArgs -IsDebug:$isDebug
#         } else {
#             _Invoke-NxRunMany -RunType $Alias -CommandArgs $finalArgs -IsDebug:$isDebug
#         }
#         return
...
#             Write-Error "Could not determine package directory name for alias '$Alias' ($nxProjectName)."; return
#         }
#         $auditArgs = @('audit', 'structure-auditor', $packageDirName)
#         _Invoke-Process -Executable 'nx' -PreArgs $auditArgs -IsDebug:$isDebug
#     } elseif ($firstArg -in 'add', 'remove') {
#         if ($IsDebug) { Write-Host "$($PSStyle.Dim)  - Routing to: Package Management (pnpm add/remove)$($PSStyle.Reset)" }
#         _Invoke-PnpmPackageManagement -PnpmPackageName $pnpmPackageName -CommandArgs $finalArgs -IsDebug:$isDebug
...
#         if ($finalArgs.Count -gt 0 -and $finalArgs -eq 'audit') {
#             if ($IsDebug) { Write-Host "$($PSStyle.Dim)  - Routing to: Full Audit Script$($PSStyle.Reset)" }
#             $auditArgs = @('scripts/audit-feature-structure.ts')
#             _Invoke-Process -Executable 'tsx' -PreArgs $auditArgs -IsDebug:$isDebug
#         } else {
#             _Invoke-NxRunMany -RunType $Alias -CommandArgs $finalArgs -IsDebug:$isDebug
#         }
...
#             Write-Error "Could not determine package directory name for alias '$Alias' ($nxProjectName)."; return
#         }
#         $auditArgs = @('scripts/audit-feature-structure.ts', $packageDirName)
#         _Invoke-Process -Executable 'tsx' -PreArgs $auditArgs -IsDebug:$isDebug
#     } elseif ($firstArg -in 'add', 'remove') {
#         if ($IsDebug) { Write-Host "$($PSStyle.Dim)  - Routing to: Package Management (pnpm add/remove)$($PSStyle.Reset)" }
#         _Invoke-PnpmPackageManagement -PnpmPackageName $pnpmPackageName -CommandArgs $finalArgs -IsDebug:$isDebug
