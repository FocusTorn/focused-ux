...
let mockDetectShellTypeCached: any = vi.fn().mockImplementation(() => {
    ...
    if (
        ...
        process.env.WSLENV ||
        (process.env.SHELL &&
            (process.env.SHELL.includes('bash') || process.env.SHELL.includes('git-bash')))
    ) {
        return 'gitbash'
    }
    return 'unknown'
...
vi.mock('./services/ConfigLoader.service.js', () => ({
    ConfigLoader: {
        getInstance: vi.fn(() => ({
            loadConfig: vi.fn().mockImplementation(() => {
                console.log(
                    `[MOCK ./services/ConfigLoader.service.js] ConfigLoader.getInstance().loadConfig called with cwd: "${process.cwd()}"`
                )
    ...
    resolveProjectForAlias: vi.fn().mockImplementation((alias) => {
        console.log(
            `[MOCK ./services/ConfigLoader.service.js] resolveProjectForAlias called with alias: "${alias}"`
        )
...
vi.mock('../../src/services/ConfigLoader.service.js', () => ({
    ConfigLoader: {
        getInstance: vi.fn(() => ({
            loadConfig: vi.fn().mockImplementation(() => {
                console.log(
                    `[MOCK ../../src/services/ConfigLoader.service.js] ConfigLoader.getInstance().loadConfig called with cwd: "${process.cwd()}"`
                )
                // const cwd = process.cwd()
                console.log(
                    `[MOCK ../../src/services/ConfigLoader.service.js] Returning valid config`
                )
...
vi.mock('../../src/services/index.js', () => {
    ...
    const expandFlags = vi
        .fn()
        .mockImplementation((args: string[], expandables: Record<string, any> = {}) => {
            const result = {
                start: [] as string[],
                prefix: [] as string[],
                preArgs: [] as string[],
    ...
    const constructWrappedCommand = vi
        .fn()
        .mockImplementation((base: string[], _start: string[], _end: string[]) => base)
    ...
    ConfigLoader: {
        getInstance: vi.fn(() => ({
            loadConfig: vi.fn().mockImplementation(() => {
                const cwd = process.cwd()
                console.log(
                    `[MOCK ../../../src/services/ConfigLoader.service.js] ConfigLoader.getInstance().loadConfig called with cwd: "${cwd}"`
                )
                if (/temp-|test-workspace/i.test(cwd)) {
                    console.log(
                        `[MOCK ../../../src/services/ConfigLoader.service.js] Throwing error due to temp workspace cwd`
                    )
...
vi.mock('../../src/services/ExpandableProcessor.service.js', () => {
    const mockProcessor = {
        ...
        constructWrappedCommand: vi
            .fn()
            .mockImplementation((base: string[], start: string[], end: string[]) => [
                ...start,
                ...base,
                ...end,
            ]),
        expandTemplate: vi
            .fn()
            .mockImplementation((template: string, variables: Record<string, string>) => {
                let result = template
        ...
        processTemplateArray: vi
            .fn()
            .mockImplementation((templates: any[], variables: Record<string, string>) => {
                const start: string[] = []
                const end: string[] = []
        ...
        constructCommand: vi
            .fn()
            .mockImplementation((args: string[], _variables: Record<string, string>) => args),
        processShellSpecificTemplate: vi
            .fn()
            .mockImplementation((expandable: any, variables: Record<string, string>) => {
