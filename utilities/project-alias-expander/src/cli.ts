...
import {
    ConfigLoader,
    clearAllCaches,
    resolveProjectForAlias as resolveProjectForAliasFromConfig,
} from './services/ConfigLoader.service.js'
...
function getContextAwareFlags(
    config: AliasConfig,
    target: string,
    expandedTarget: string
): Record<string, import('./_types/expandable.types.js').ExpandableValue> {
    //>
...
async function loadPAEModule() {
    ...
    try {
        ...
        try {
            const { stdout } = await execa('pwsh', ['-Command', '$PROFILE.CurrentUserAllHosts'], {
                stdio: 'pipe',
            })
...
function resolveProjectForAlias(
    aliasValue: string | { name: string; suffix?: 'core' | 'ext'; full?: boolean }
): { project: string; full?: boolean } {
...
const isMainModule =
    normalizedImportUrl === normalizedArgvPath ||
