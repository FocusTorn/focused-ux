        ...
        try {
            ...
            if (helpConfig.nxPackages && Object.keys(helpConfig.nxPackages).length > 0) {
                ...
                Object.entries(helpConfig.nxPackages).forEach(([alias, project]) => {
                    if (alias !== 'desc') {
                        const projectName =
                            typeof project === 'string' ? project : (project as any).name
            ...
            if (
                helpConfig.targets?.['nx-targets'] &&
                Object.keys(helpConfig.targets['nx-targets']).length > 0
            ) {
            ...
            if (
                helpConfig['feature-nxTargets'] &&
                Object.keys(helpConfig['feature-nxTargets']).length > 0
            ) {
                ...
                Object.entries(helpConfig['feature-nxTargets']).forEach(([alias, config]) => {
                    if (alias !== 'desc') {
                        console.log(
                            `  ${alias.padEnd(8)} → ${(config as any)['run-target']} (from ${(config as any)['run-from']})`
                        )
                    }
                })
                console.log('')
            ...
            if (
                helpConfig['expandable-flags'] &&
                Object.keys(helpConfig['expandable-flags']).length > 0
            ) {
                ...
                Object.entries(helpConfig['expandable-flags']).forEach(([flag, expansion]) => {
                    if (flag !== 'desc') {
                        const expansionStr =
                            typeof expansion === 'string' ? expansion : (
                                (expansion as any).template || 'template'
                            )
            ...
            if (
                helpConfig['expandable-templates'] &&
                Object.keys(helpConfig['expandable-templates']).length > 0
            ) {
                ...
                Object.entries(helpConfig['expandable-templates']).forEach(([template, config]) => {
                    if (template !== 'desc') {
                        console.log(
                            `  -${template.padEnd(8)} → ${(config as any)['pwsh-template'] ? 'PowerShell template' : 'Template'}`
                        )
                    }
                })
                console.log('')
            ...
            if (
                helpConfig['expandable-commands'] &&
                Object.keys(helpConfig['expandable-commands']).length > 0
            ) {
            ...
            if (
                ...
                console.log(`expandable Commands: ${dimmed}${desc}${reset}`)
                Object.entries(helpConfig['expandable-commands']).forEach(
                    ([command, execution]) => {
