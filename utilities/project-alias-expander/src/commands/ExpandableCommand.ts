...
export class ExpandableCommand {
    ...
    async execute(alias: string, args: string[], config: AliasConfig): Promise<number> {
        ...
        this.debug('About to check env-setting-flags', {
            hasEnvSettingFlags: !!config['env-setting-flags'],
            envSettingFlagsKeys:
                config['env-setting-flags'] ? Object.keys(config['env-setting-flags']) : [],
        })

        if (config['env-setting-flags']) {
            this.debug('Processing env-setting flags', {
                envSettingFlags: config['env-setting-flags'],
            })
            ...
            const {
                ...
                remainingArgs: envRemainingArgs,
            } = expandableProcessor.expandFlags(args, envSettingFlags)

            this.debug('Env-setting flags processed', {
                envStart,
                envPrefix,
                envPreArgs,
                envSuffix,
            ...
            for (const arg of allEnvProcessedArgs) {
                if (arg === '--pae-debug') {
                    ...
                    if (variant) {
                        ...
                        this.debug(
                            'Echo mode enabled via --pae-echo flag (no variant - will show all)'
                        )
                    }
                    ...
                    if (variant) {
                        process.env.PAE_ECHO_VARIANT = variant
                        this.debug(
                            `echoX mode enabled via --pae-echoX flag with variant: ${variant}`
                        )
                    } else {
                        this.debug(
                            'EchoX mode enabled via --pae-echoX flag (no variant - will show all)'
                        )
                    }
        ...
        if (config['internal-flags']) {
            ...
            const { remainingArgs: internalRemainingArgs } = expandableProcessor.expandFlags(
                args,
                internalFlags
            )
        ...
        const {
            ...
            remainingArgs,
        } = expandableProcessor.expandFlags(args, expandableFlags)
