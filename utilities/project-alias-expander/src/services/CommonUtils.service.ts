...
const DEBUG =
    process.env.PAE_DEBUG === '1' || process.argv.includes('-d') || process.argv.includes('--debug')
...
export class ErrorHandler {
    static handleExecutionError(error: any, context: string): number {
        ...
        if (error.timedOut) {
            // Extract timeout from error message or use duration as fallback
            let timeoutMs: number | string = 'unknown'
            let timeoutSeconds: number | string = 'unknown'

            if (error.timeout) {
                timeoutMs = error.timeout
                timeoutSeconds = Math.round(Number(timeoutMs) / 1000)
            } else if (error.durationMs) {
                timeoutMs = error.durationMs
                timeoutSeconds = Math.round(Number(timeoutMs) / 1000)
            } else {
                // Try to extract from error message
                const match = error.message?.match(/(\d+) milliseconds/)
                if (match) {
                    timeoutMs = parseInt(match[1])
                    timeoutSeconds = Math.round(Number(timeoutMs) / 1000)
                }
            }

            console.error(
                `\n\x1b[31m⏰ PAE Timeout Exceeded: ${timeoutSeconds}s (Command: ${error.command || 'unknown'})\x1b[0m`
            )
    ...
    static handleTemplateError(
        ...
        console.warn(
            `Template processing failed for "${template}" with variables:`,
            variables,
            error
        )
    ...
    static async executeNxCommand(argv: string[], timeoutMs?: number): Promise<number> {
        ...
        try {
            ...
            if (argv[0]?.startsWith('$')) {
                return ProcessUtils.executeCommand('powershell', ['-Command', argv.join(' ')], {
                    timeout: timeoutMs,
                })
            }
...
export class TemplateUtils {
    ...
    static applyMutation(value: any, mutation: string): any {
        try {
            ...
            console.warn(
                `Mutation failed for value "${value}" with expression "${mutation}":`,
                error
            )
    ...
    private static evaluateExpression(value: any, expression: string): any {
        ...
        if (
            ...
            expression.includes('parseInt(') &&
            expression.includes("+ '000')")
        ) {
            return value >= 100 ? value : parseInt(value.toString() + '000')
        }
        ...
        console.warn(
            `Unsupported mutation expression: "${expression}". Please add support for this pattern.`
        )
        return value
    }
    ...
    static applyPowerShellReplacements(value: any, mutation: string): any {
        ...
        while ((match = replaceRegex.exec(mutation)) !== null) {
            ...
            console.log(
                `[DEBUG] Processing replacement: pattern="${pattern}", replacement="${replacement}"`
            )
        ...
        if (templateDefaults) {
            ...
            for (const key of Object.keys(templateDefaults)) {
                if (key in templateVariables) {
                    throw new Error(
                        `Variable conflict: '${key}' is defined in both top-level and template-level defaults`
                    )
                }
            }
...
export class ConfigUtils {
     ...
     */
    static resolveProjectForAliasV2(
        aliasValue:
            | string
            | { name: string; suffix?: 'core' | 'ext'; full?: boolean }
            | { aliases: string[]; variants: Record<string, string>; default?: string },
        config: any
    ): { project: string; isFull: boolean } {
        ...
        if ('aliases' in aliasValue && 'variants' in aliasValue) {
            const packageDef = aliasValue as {
                aliases: string[]
