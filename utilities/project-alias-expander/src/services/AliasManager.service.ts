...
export class AliasManagerService implements IAliasManagerService {
    ...
    async generateDirectToNativeModules(): Promise<void> {
        ...
        try {
            ...
            const psModuleDirs = psModulePath
                .split(';')
                .filter(
                    (dir) =>
                        dir.includes('PowerShell') &&
                        dir.includes('Modules') &&
                        !dir.includes('WindowsPowerShell')
                )
            const baseModuleDir =
                psModuleDirs[0] ||
                path.join(
        ...
        try {
            ...
            if (isVerbose) {
                console.log(
                    `\x1b[32m✅ Bash script generated directly to: ${bashScriptPath}\x1b[0m`
                )
            }
        } catch (error) {
            if (isVerbose) {
     ...
     */
    private generateScriptContent(aliases: string[]): {
    ...
    async installAliases(): Promise<void> {
        ...
        if (isVerbose) {
            ...
            console.log(
                '\x1b[36m📁 PowerShell Module:\x1b[0m libs/project-alias-expander/dist/pae-functions.psm1'
            )
            console.log(
                '\x1b[36m📁 Bash Script:\x1b[0m libs/project-alias-expander/dist/pae-aliases.sh'
            )
            ...
            if (shell === 'powershell') {
                ...
                console.log(
                    '   Import-Module libs/project-alias-expander/dist/pae-functions.psm1 -Force'
                )
