...
export class TargetResolutionService {
    ...
    resolveTarget(
        target: string,
        config: AliasConfig,
        isFeatureLevel: boolean = false
    ): { target: string; flags: string[] } {
        let resolvedTarget: string
        let flags: string[] = []

        // Check feature targets first only if this is a feature-level alias
        ...
        } else if (config.targets?.['not-nx-target']?.[target]) {
            resolvedTarget = config.targets['not-nx-target'][target]
        } else {
            // Return target as-is if not found (might be a custom target)
            resolvedTarget = target
        }

        return { target: resolvedTarget, flags }
    }
    ...
    validateTargetStructure(config: AliasConfig): string[] {
        ...
        if (config.targets) {
            if (!config.targets['nx-targets'] && !config.targets['not-nx-target']) {
                errors.push(
                    'Targets structure must contain at least one of "nx-targets" or "not-nx-target"'
                )
            }
            ...
            if (duplicates.length > 0) {
                errors.push(
                    `Duplicate targets found between nx-targets and not-nx-target: ${duplicates.join(', ')}`
                )
            }
        }
