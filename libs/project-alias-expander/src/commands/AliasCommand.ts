...
import { CommandResolutionService, type CommandResolution } from '../services/CommandResolutionService.js'
...
export class AliasCommand {
    ...
    async execute(args: string[], config: AliasConfig): Promise<number> {
        try {
            ...
            switch (resolution.type) {
                case 'reserved':
                    return await this.handleReservedCommand(resolution.command, remainingArgs, config)
                case 'expandable':
                    return await this.handleExpandableCommand(resolution.execution!, remainingArgs, config)
                case 'package':
    ...
    private async handleReservedCommand(command: string, args: string[], config: AliasConfig): Promise<number> {
    ...
    private async handleExpandableCommand(execution: string, args: string[], config: AliasConfig): Promise<number> {
    ...
    private async handlePackageAlias(alias: string, args: string[], config: AliasConfig): Promise<number> {
        try {
            ...
            const expandedTarget = this.targetResolver.resolveTarget(target, config)
            ...
            return await this.processFlagsAndExecute(packageResolution, expandedTarget, args, config, expandableFlags)
    ...
    private async processFlagsAndExecute(
        ...
        if (config['env-setting-flags']) {
            ...
            expandedFlags = [...expandedFlags, ...expansion.start, ...expansion.prefix, ...expansion.preArgs, ...expansion.suffix, ...expansion.end]
        ...
        if (config['expandable-flags']) {
            this.debug('Processing expandable flags', { processedArgs, expandableFlags: config['expandable-flags'] })
            try {
                const expansion = expandableProcessor.expandFlags(processedArgs, config['expandable-flags'])
                this.debug('Expandable flags expansion result', { expansion })
                processedArgs = expansion.remainingArgs
                expandedFlags = [...expandedFlags, ...expansion.start, ...expansion.prefix, ...expansion.preArgs, ...expansion.suffix, ...expansion.end]
                this.debug('Updated expanded flags', { expandedFlags, processedArgs })
            } catch (err) {
                this.error('Flag processing error:', err instanceof Error ? err.message : String(err))
                return 1
            }
        }

        // Process context-aware flags
        ...
        // Combine all arguments: expanded flags + remaining args
        const finalArgs = [...expandedFlags, ...processedArgs]
        ...
        this.debug('Executing command', { command, isTool, packageResolution })

        return await commandExecution.runCommand('nx', commandArgs, timeoutMs)
    }

    private processInternalFlags(args: string[], config: AliasConfig): { remainingArgs: string[], timeout?: number } {
        ...
        for (const arg of args) {
            if (arg === '--help' || arg === '-h') {
                ...
                timeout = timeoutValue >= 100 ? timeoutValue : parseInt(timeoutValue.toString() + '000')
            } else {
