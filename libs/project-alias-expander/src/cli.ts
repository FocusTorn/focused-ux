...
// When built, the CLI is in dist/cli.js, so we need to go up two levels to reach the package root
...
function loadAliasConfig(): AliasConfig {
	// The config.json is in the project-alias-expander directory
	const configPath = path.join(PACKAGE_ROOT, 'project-alias-expander', 'config.json')
...
function runMany(runType: 'ext' | 'core' | 'all', targets: string[], flags: string[], config: AliasConfig): number {
	...
	if ((target === 'test:full' || target === 'validate:full' || target === 'lint:full')
	  && !enhancedFlags.some(flag => flag === '--stream' || flag === '--output-style=stream' || flag.startsWith('--output='))) {
...
function main() {
	...
	if ((normalizedTarget === 'test:full' || normalizedTarget === 'validate:full' || normalizedTarget === 'lint:full')
	  && !enhancedArgs.some(arg => arg === '--stream' || arg === '--output-style=stream' || arg.startsWith('--output='))) {
		enhancedArgs = ['--output-style=stream', ...enhancedArgs]
	}

	// Auto-inject --parallel=false for validate:full to get cleaner sequential output
	if (normalizedTarget === 'validate:full' && !enhancedArgs.some(arg => arg === '--parallel=false' || arg === '--parallel=true')) {
