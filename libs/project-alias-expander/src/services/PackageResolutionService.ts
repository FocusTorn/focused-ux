...
export class PackageResolutionService {
            ...
            if (typeof value === 'object' && 'aliases' in value) {
                ...
                Object.entries(value).forEach(([subKey, subValue]) => {
                    if (typeof subValue === 'string') {
                        ...
                        throw new Error(`Invalid package definition format for '${key}.${subKey}'. Expected string.`)
                    }
                })
            } else {
                throw new Error(`Invalid package definition format for '${key}'. Expected PackageDefinition object, string, or object with string values.`)
            }
        })
    }
    ...
    private buildFullName(packageName: string, packageDef: PackageDefinition, variant?: string): string {
        if (variant) {
