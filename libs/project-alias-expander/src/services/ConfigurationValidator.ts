...
export class ConfigurationValidator {
        ...
        Object.entries(config['expandable-flags'] || {}).forEach(([key, value]) => {
            if (typeof value === 'object' && value.template) {
                if (value.template.includes('{') && !value.template.includes('{{')) {
                    errors.push(`Template '${key}' uses old syntax '{variable}'. Use '{{variable}}' instead.`)
                }
            }
        })

        // Check context-aware flags
        Object.entries(config['context-aware-flags'] || {}).forEach(([contextKey, contextValue]) => {
            Object.entries(contextValue).forEach(([key, value]) => {
                if (typeof value === 'object' && value.template) {
                    if (value.template.includes('{') && !value.template.includes('{{')) {
                        errors.push(`Template '${contextKey}.${key}' uses old syntax '{variable}'. Use '{{variable}}' instead.`)
                    }
                }
            })
        })
    ...
    private validateCommandConflicts(config: AliasConfig): string[] {
        ...
        if (conflicts.length > 0) {
            errors.push(`Package aliases cannot be used as expandable commands: ${conflicts.join(', ')}`)
        }
    ...
    isOldFormat(config: any): boolean {
        return (
            ...
            (config.nxPackages && Object.values(config.nxPackages).some((value: any) => 
                typeof value === 'object' && value.name && value.suffix
            ))
        )
    }
}
