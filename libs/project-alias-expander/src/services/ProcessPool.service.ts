...
export class ProcessPool {
    ...
    private handleProcessExit(pid: number, code: number | null, signal: string | null, startTime: number): void {
        ...
        if (this.config.enableMetrics) {
            console.debug(`Process ${pid} exited with code ${code}, signal ${signal}, duration ${duration}ms`)
        }
    }
    ...
    private async performShutdown(timeoutMs: number): Promise<void> {
        ...
        const shutdownPromises = Array.from(this.activeProcesses.values()).map(async (activeProcess) => {
            try {
                if (!activeProcess.process.killed) {
