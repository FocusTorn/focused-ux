...
export default async function runExecutor(options: PackExecutorSchema, context: ExecutorContext): Promise<Result> {
    try {
        ...
        // deployPath: .vpack/deploy, keepDeploy (persistent): true, freshDeploy (overwrite): true,
        // extractContents: true, contentsPath: .vpack/contents, outputPath: vsix_packages
        const deployBase = options.deployPath || '.vpack/deploy'
        ...
        const extractBase = options.contentsPath || '.vpack/contents'
        const finalOutputDir = join(workspaceRoot, options.outputPath || 'vsix_packages')
        ...
        if (options.dev) {
            ...
            ;(finalPackageJson as any).version = finalVersion
        }
            ...
            for (const depName in dependencies) {
                ...
                if (depInfo?.path) {
                    ...
                    if (!existsSync(destPath)) {
                        ...
                        try { cpSync(depInfo.path, destPath, { recursive: true }) } catch (err: any) {
