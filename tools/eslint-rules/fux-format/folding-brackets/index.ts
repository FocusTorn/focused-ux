...
const rule: ESLint.Rule.RuleModule = {
    ...
    create(context) {
        ...
        function check(node: any) {
            ...
            if (props.length === 0) return // No properties to format
            ...
            const foldingMarkerComment = source.getCommentsInside(node).find((comment: any) =>
        ...
        function checkCurrentFormat(node: any, props: any[], open: any, close: any, targetFormat: string, hasFoldingMarker: boolean, foldingMarkerComment: any) {
            const firstProp = source.getFirstToken(props[0])
            const firstPropInline = open.loc.end.line === firstProp.loc.start.line
            ...
            if (targetFormat === 'single') {
                ...
                if (hasFoldingMarker) return true
                return !props.every(prop => {
                    const propToken = source.getFirstToken(prop)
                ...
                if (!markerOnFirstLine) return true
                ...
                if (textBetweenOpenAndFirstProp !== '   ') return true
        ...
        function generateFix(node: any, props: any[], open: any, close: any, targetFormat: string, hasFoldingMarker: boolean, foldingMarkerComment: any) {
            if (targetFormat === 'single') {
                ...
                return {
                    range: [open.range[1], close.range[0]] as [number, number],
                    text: ` ${inlineProps} `
                }
            } else if (targetFormat === 'standard') {
